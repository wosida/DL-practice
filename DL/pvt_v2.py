...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., linear=False):
        ...
        self.linear = linear
        if self.linear:   #linear决定是否使用ReLU
    ...
    def forward(self, x, H, W):  #前馈层中加一个深度卷积代替位置编码
        x = self.fc1(x)
        if self.linear:
            x = self.relu(x)
        x = self.dwconv(x, H, W)
...
a=torch.randn(1,4096,512)
att=Attention(512,linear=True)
x=att(a,64,64)
print(x.shape)
...
class OverlapPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x) #(b,c,h,w)->(b,stride2*c,h/stride,w/stride)
        print(x.shape)
        _, _, H, W = x.shape
        ...
        return x, H, W
x=torch.randn(1,3,512,512) #(b,c,H,W)
patch_embed=OverlapPatchEmbed(embed_dim=200,patch_size=12,stride=8)   #embed_dim=stride2*c(C是想要输出的C)  ,patch_size-stride=2的倍数
x,h,w=patch_embed(x)
...
print(x.shape)
#复原
x=Rearrange('b (h w) (p1 p2 c) -> b c (h p1) (w p2)', h=h,w=w ,p1=8, p2=8)(x)
print(x.shape)
